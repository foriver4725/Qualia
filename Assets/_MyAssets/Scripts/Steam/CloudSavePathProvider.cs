...
namespace MyScripts.Steam
{
    ...
    {
        /// <summary>
        /// <para>Steam Cloud のセーブデータ保存先パスを取得する</para>
        ...
        /// <param name="fileName">セーブデータのファイル名 (拡張子なし)</param>
        /// <returns>%UserProfile%/AppData/LocalLow/[CompanyName]/[ProductName]/SaveData/Steam/[SteamUserID]</returns>
        public static string Create(string persistentDataPath, string fileName)
        {
            // 64bit
            ulong steamUserId = SteamUser.GetSteamID().m_SteamID;
            return Path.Combine(persistentDataPath, "SaveData", "Steam", steamUserId.ToString(), fileName + ".dat");
        }
    }
}
